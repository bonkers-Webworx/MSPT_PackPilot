...
def launch_gui():
    ...
    header_font = tkfont.Font(
        family="Segoe UI", size=settings.get("font_size", 12) + 2, weight="bold"
    )
    ...
    ctk.CTkButton(
        root, text="Browse", command=lambda: source_var.set(filedialog.askdirectory())
    ).grid(row=0, column=2)

    ctk.CTkLabel(root, text="Destination Folder").grid(
    ...
    ctk.CTkButton(
        root,
        text="Browse",
        command=lambda: destination_var.set(filedialog.askdirectory()),
    ).grid(row=1, column=2)

    ctk.CTkLabel(root, text="Structure").grid(
        row=2, column=0, padx=10, pady=5, sticky="w"
    )
    ctk.CTkComboBox(
        root, variable=structure_var, values=["Flat", "Nested"], width=400
    ).grid(row=2, column=1)
    ...
    ctk.CTkComboBox(
        root,
        variable=naming_var,
        values=["Pack-MIDI", "Genre-Pack", "Folder Only"],
        width=400,
    ).grid(row=3, column=1)
    ctk.CTkButton(root, text="ℹ️", width=30, command=show_naming_help).grid(
        row=3, column=2, padx=5
    )

    ctk.CTkLabel(root, text="Genre Filter").grid(
        row=4, column=0, padx=10, pady=5, sticky="w"
    )
    ctk.CTkComboBox(
        root,
        variable=genre_filter_var,
        values=["All", "HipHop", "Trap", "EDM", "House"],
        width=400,
    ).grid(row=4, column=1)

    ctk.CTkLabel(root, text="Pack Name Filter").grid(
    ...
    ctk.CTkComboBox(
        ...
        values=["Flat", "Nested by Genre", "Nested by Pack"],
        width=400,
    ).grid(row=6, column=1)

    ctk.CTkLabel(root, text="Font Size").grid(
        row=7, column=0, padx=10, pady=5, sticky="w"
    )
    font_slider = ctk.CTkSlider(
        root, from_=10, to=24, variable=font_size_var, number_of_steps=14
    )
